...
# TODO: Add patterns for other SE sites such as Ask Ubuntu, Server Fault, Super User

url_patterns = [url_pattern_reddit, url_pattern_so, url_pattern_se, url_pattern_hn]
...
class InvalidUrl(InvalidInputError):
    '''Invalid URL'''
    def __init__(self):
        self.message = 'Please enter a URL prefixed with "https://".\nSupported sites: Reddit, Hacker News, Stack Exchange (incl. Stack Overflow)'
        super().__init__(self.message)

class InvalidThreadId(InvalidInputError):
    '''Invalid thread ID'''
    def __init__(self):
        self.message = 'Please enter a valid thread ID (positive integer). Run `filum all` to see a list of thread IDs.'
        super().__init__(self.message)
